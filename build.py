...
def get_llvm_project(version, tag):
    ...
    if version in ["7", "8"]:
        subprocess.run(f"curl -LO {RELEASE_URL}/llvmorg-{tag}/llvm-{tag}.src.tar.xz", shell=True, check=True)
        subprocess.run(f"curl -LO {RELEASE_URL}/llvmorg-{tag}/cfe-{tag}.src.tar.xz", shell=True, check=True)
        subprocess.run(f"curl -LO {RELEASE_URL}/llvmorg-{tag}/clang-tools-extra-{tag}.src.tar.xz", shell=True, check=True)
    else:
        subprocess.run(f"curl -LO {RELEASE_URL}/llvmorg-{tag}/llvm-project-{tag}.src.tar.xz", shell=True, check=True)
...
def unpack_llvm_project(version, tag):
    ...
    if version in ["7", "8"]:
        ...
        subprocess.run(f"tar xf clang-tools-extra-{tag}.src.tar.xz", shell=True, check=True)
    else:
...
def build_project(version, platform):
    ...
    subprocess.run(f"cmake -S llvm-{version} -B llvm-{version}/build {COMMON_CMAKE_ARGS} {cmake_args}", shell=True, check=True)
    subprocess.run(f"cmake --build llvm-{version}/build {build_args}", shell=True, check=True)

def main():
    parser = argparse.ArgumentParser(description="Build LLVM for multiple versions and platforms.")
    parser.add_argument("-v", "--version", nargs="*", help="Specify LLVM versions to build.")
    parser.add_argument("-p", "--platform", nargs="*", choices=PLATFORMS, help="Specify platforms to build for.")
